...
void solve()
{
	...
	bool isAlarmSleepSame = false;

	lp(i, n) {
		inpp(a[i].first, a[i].second);
		isAlarmSleepSame = (a[i].first == H and a[i].second == M);
	}
	...
	int ans = 24 * 60, ansh, ansm;

	lp(i, n) {
		int al = a[i].first * 60 + a[i].second;
