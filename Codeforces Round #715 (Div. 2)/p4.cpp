...
void solve()
{
	...
	int lans = 3*n;
	int ai=0, bi=0, ci=0;
	while(lans > 0 and ai<2*n and bi<2*n and ci<2*n)
	{
		if(a[ai]==b[bi])
		{
			if(a[ai]==c[ci])
	...
	if(ai != 2*n)
	{
		if(ci == 2*n and bi == 2*n)
		{
			goto label;
		}
		else if(ci != 2*n)
		{
			while(lans > 0 and ai<2*n and ci<2*n)
			{
				if(a[ai]==c[ci])
				{
					ans.push_back(a[ai]);
		...
		{
			while(lans > 0 and ai<2*n and bi<2*n)
			{
				if(a[ai]==b[bi])
				{
					ans.push_back(a[ai]);
	...
	{
		if(ci == 2*n and ai == 2*n)
		{
			goto label;
		}
		else if(ci != 2*n)
		{
			while(lans > 0 and bi<2*n and ci<2*n)
			{
				if(b[bi]==c[ci])
				{
					ans.push_back(b[bi]);
