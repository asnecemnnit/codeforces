...
bool isSymmetric(int a, int b, int c, int d)
{
	if(b == c)
		return true;
	return false;
...
// bool isTranspose(int a, int b, int c, int d, int l, int m, int n, int 0)
// {
// 	if()
// }
...
void solve()
{
	...
	for(int i=0; i<n; i++)
	{
		INP2(tiles[i][0][0], tiles[i][0][1]);
		INP2(tiles[i][1][0], tiles[i][1][1]);
	}
	...
	for(int i=0; i<n; i++)
	{
		if(isSymmetric(tiles[i][0][0], tiles[i][0][1], tiles[i][1][0],
			tiles[i][1][1]))
		{
			OUT("YES");
			return;
