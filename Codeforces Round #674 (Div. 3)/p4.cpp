...
/**************************************/
vector<pair<int, int> > mxPairs;
vector<pair<int, int> > mxUnqPairs;
// create an empty map
unordered_map<long long int, vector<int> > hmap;
// unordered_map<int, int> hmap;

/******** User-defined Function *******/
void findSubArrays(vector<int> arr, int n)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        if (sum == 0)
            mxPairs.push_back(make_pair(0, i));
        ...
        {
            // hmap[sum] stores starting index of all subarrays
            vector<int> vc = hmap[sum];
            for (auto it = vc.begin(); it != vc.end(); it++)
                mxPairs.push_back(make_pair(*it + 1, i));
			hmap.erase(sum);
        }
...
void maxDisjointIntervals()
{

	mxUnqPairs.clear();
    // Sort the mxPairs of intervals
    sort(mxPairs.begin(), mxPairs.end(), sortbysec);

	mxUnqPairs.emplace_back(mxPairs[0].first, mxPairs[0].second);

    // End point of first interval
    int r1 = mxPairs[0].second;

    for (int i = 1; i < mxPairs.size(); i++) {
        int l1 = mxPairs[i].first;
        int r2 = mxPairs[i].second;
...
void solve()
{
	...
	// vector<pair<int, int> > out = maxDisjointIntervals(out);
	maxDisjointIntervals();

	// REP(i,maxUniquePairs.size())
	// {
	// 	cout<<maxUniquePairs[i].first<<" "<<maxUniquePairs[i].second<<endl;
	// }
