...
int maxWinRounds(int a1, int a2, int a3, int b1, int b2, int b3)
{
	int cnt=0;
	...
	return min(a3,b1) + min(a1,b2) + min(b3,a2);
}

int minWinRounds(int a1, int a2, int a3, int b1, int b2, int b3 , int n)
{
	int cnt=0, ans = INT_MAX;

	vector<pair<int,int>> ab_not_winning = {{2,1}, {0,2}, {1,0}, {0,0}, {1,1}, {2,2}};
	sort(ab_not_winning.begin(), ab_not_winning.end());
	...
	do {
		...
		for(int i=0; i<6; i++)
		{
			// cout<<a[ab_not_winning[i].first]<<" "<<b[ab_not_winning[i].second]<<endl;

			cnt = min(a[ab_not_winning[i].first], b[ab_not_winning[i].second]);
			a[ab_not_winning[i].first] -= cnt;
			b[ab_not_winning[i].second] -= cnt;
	...
	} while(next_permutation(ab_not_winning.begin(), ab_not_winning.end()));
...
void solve()
{
	...
	int mn = minWinRounds(a1, a2, a3, b1, b2, b3, n);
	int mx = maxWinRounds(a1, a2, a3, b1, b2, b3);
	OUT2(mn, mx);
	return;
