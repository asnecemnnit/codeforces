...
void solve()
{
	...
	vector<int> dp(n);
	bool found;
	int hmin = INT_MAX, wmin = INT_MAX, mn = INT_MAX;
	REP(i,n)
	{
		if((hw[i].first > hmin and hw[i].second > wmin) or
		(hw[i].first > wmin and hw[i].second > hmin))
		{
			dp[i] = mn;
			continue;
		}
		...
		REP(j,n)
		{
			...
			if((hw[j].first < hw[i].first and hw[j].second < hw[i].second) or
			(hw[j].first < wh[i].first and hw[j].second < wh[i].second))
			{
				if(hw[j].first < hmin and hw[j].second < wmin)
				{
					hmin = hw[j].first;
					wmin = hw[j].second;
					mn = j+1;
		...
		if(!found)
			dp[i] = -1;
		// cout<<hw[i].first<<" "<<hw[i].second<<endl;
	}
