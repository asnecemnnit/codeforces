...
void solve()
{
	...
	int e_start = 0, o_start = 0, e_end = ne-1, o_end = no-1;
	bool e_turn = true;
	long long int sce = 0, sco = 0;

	while(e_start <= e_end and o_start <= o_end)
	{
		// cout<<"e_turn"<<e_turn<<endl;
		if(e_turn)
		{
			if(ae[e_end] < ao[o_end])
			{
				o_end--;
			}
		...
		{
			if(ao[o_end] < ae[e_end])
			{
				e_end--;
			}
			else
			{
				sco += (long long int)ao[o_end];
				o_end--;
			}
		}
		// cout<<"sce"<<sce<<"sco"<<sco<<endl;
