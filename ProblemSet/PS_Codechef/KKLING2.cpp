...
//        cout<<"\n";
        ...
        if (j < leaves.size())
        {
            v.clear();
            int sz = leaves.size();
            int val = leaves[sz - 1].first;
            v.emplace_back(p.first + 1, leaves[sz - 1].second);

            int kk = sz - 2;
            while (kk >= 0 and leaves[kk].first == val) {
                v.emplace_back(p.first + 1, leaves[kk].second);
                kk--;
            }

            for (int ll = j; ll <= kk; ll++)
            {
                if (leaves[ll].first == p.first + 1)
                {
                    v.emplace_back(p.first + 1, leaves[ll].second);
                }
            }
        }
        leaves.clear();
