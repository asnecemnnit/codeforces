...
void Graph::DFS(int v, int u)
{
	...
	for (int i = 0; i < adj[v].size(); i++) {
		...
		{
			DFS(node, v);
			dp[v] += ((2 * dp[node]) % mod), dp[v] %= mod, cnt++, total[v] += total[node], total[v] %= mod, total[v] += dp[node], total[v] %= mod;
			sum += dp[node];
		}
	}
	...
	for (int i = 0; i < adj[v].size(); i++) {
		...
		if (node != u)
		{
			total[v] += (dp[node] * ((sum - dp[node] + mod) % mod)) % mod;
			total[v] %= mod;
		}
	}
}
