...
int n;
const int NMAX = 500;
mint dp[NMAX][NMAX * (NMAX + 1) / 2];
mint ans;

/******* User-defined Functions *******/
/*	SUM(0 to j){dp[i][j]} = dp[i-1][0] + dp[i-1][1] + dp[i-1][2] + ..... + dp[i-1][j] +
				 dp[i-1][0] + dp[i-1][1] + dp[i-1][2] +	..... dp[n-1][j-i]
	ANS = dp[n-1][sum] = dp[n-1][sum] (excluding current n) + dp[n-1][sum-n] (including current n)
*/
mint helper_iterative(int sum) {
	FOR(i, 1, n, 1) {
		REP(j, sum + 1) {
			dp[i][j] = dp[i - 1][j];
			if (j >= i) {
