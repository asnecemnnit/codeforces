...
int lis()
{
	...
	{
		// O(logn)
		auto left = lower_bound(pilesTop.begin(), pilesTop.end(), x[i]);
		if (left == pilesTop.end())	/* Append to the end */
			pilesTop.push_back(x[i]);
		else	/* Replace with the value in pilesTop just greater than x[i] */
			*left = x[i];
