...
const int NMAX = 5000;
int n, m;
long long int dp_s1_minus_s2[NMAX][NMAX];

/******* User-defined Functions *******/
void helper(vector<int> x) {

	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			if (i == j)
				dp_s1_minus_s2[i][j] = x[i];
			else
				dp_s1_minus_s2[i][j] = 0;
		}
	}

	/* 	dp[n-2][n-1] = max(x[n-2] - dp[n-1][n-1], x[n-1] - dp[n-2][n-2])					....... A
		dp[n-3][n-2] = max(x[n-3] - dp[n-2][n-2], x[n-2] - dp[n-3][n-3])					.......	B
		dp[n-3][n-1] = max(x[n-3] - dp[n-2][n-1] {from A}, x[n-1] - dp[n-3][n-2] {from B})
		.
		.
		.
		.
		.
		dp[0][n-3] = max(x[0] - dp[1][n-3], x[n-3] - dp[0][n-4])							.......	Y
		dp[0][n-2] = max(x[0] - dp[1][n-2], x[n-2] - dp[0][n-3] {from Y})					.......	Z
		dp[0][n-1] = max(x[0] - dp[1][n-1], x[n-1] - dp[0][n-2] {from Z})
	*/

	for (int i = n - 2; i >= 0; i--) {
		for (int j = i + 1; j < n; j++) {
			dp_s1_minus_s2[i][j] = max(x[i] - dp_s1_minus_s2[i + 1][j], x[j] - dp_s1_minus_s2[i][j - 1]);
		}
	}
...
void solve()
{
	...
	OUT((dp_s1_minus_s2[0][n - 1] + sum) / 2);
	return;
}
