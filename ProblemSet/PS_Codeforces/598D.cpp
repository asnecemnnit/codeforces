...
void calc_max_pictures(vector<vector<char>> b, int n, int m) {
  lp(x, n) {
    if (x == 0 or x == n - 1)
      continue;
    lp(y, m) {
      ...
      while (q.size() > 0) {
        ...
        lp(k, 4) {
          ...
          if (nbx == 0 or nby == 0 or nbx == n - 1 or nby == m - 1 or
              (b[nbx][nby] == '*')) {
            cnt++;
            continue;
          }

          if (visited[nbx][nby] == -1 and (b[nbx][nby] == '.')) {
            visited[nbx][nby] = 1;
            q.push(make_pair(nbx, nby));
            parent[nbx][nby] = {parent[x][y].first, parent[x][y].second};
          }
        }
      }
      dp[parent[x][y].first][parent[x][y].second] = cnt;
    }
  }
  return;
}
...
void solve() {
  ...
  calc_max_pictures(b, n, m);
  lp(idx, k) {
    int x, y;
    inpp(x, y);
    --x, --y;
    out(dp[parent[x][y].first][parent[x][y].second]);
  }
