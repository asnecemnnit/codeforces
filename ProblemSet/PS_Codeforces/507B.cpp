...
int dist(pii a, pii b) {
	dbg4(a.first, a.second, b.first, b.second);
	uint64 sqr = (uint64)abs(a.first - b.first) * (uint64)abs(a.first - b.first) +  (uint64)abs(a.second - b.second) * (uint64)abs(a.second - b.second);
	dbg(sqr);
	if (sqr > sqrtl(sqr) * sqrtl(sqr))
		return sqrtl(sqr) + 1;
	return sqrtl(sqr);
}
...
void solve()
{
	...
	int distance = dist(mpr(x, y), mpr(x1, y1));
