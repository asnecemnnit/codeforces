...
void solve()
{
	...
	vector<vector<int>> uvx(n - 1, vector<int>(3));
	int cnt = 0;
	lp(i, n - 1) {
		inppp(uvx[i][0], uvx[i][1], uvx[i][2]);
		if (mp.find(uvx[i][0]) == mp.end()) {
			// Key does not exist, add the key-value pair
			mp.insert(mpr(uvx[i][0], cnt++));
		}

		if (mp.find(uvx[i][1]) == mp.end()) {
			// Key does not exist, add the key-value pair
			mp.insert(mpr(uvx[i][1], cnt++));
		}
	}

	DisjointSet ds(mp.size());

	lp(i, n - 1) {
		if (uvx[i][2] == 0) {
			ds.Union(mp[uvx[i][0]], mp[uvx[i][1]]);
		}
	}

	mint ans = (mint)(mp.size()) ^ (mint)(k);

	unordered_map<int, int> sizes = ds.getSizes();
	for (const auto& [representative, size] : sizes) {
		ans -= (mint)(size) ^ (mint)(k);
	}
