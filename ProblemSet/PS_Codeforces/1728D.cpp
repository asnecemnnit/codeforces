...
void solve()
{
	...
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			dp[i][j] = -1;//initialize with loss of Alice (impossible???)
		}
	}
	...
	while (substrlen <= n) {
		for (int i = 0; i + substrlen - 1 < n; i++) {
			...
			if (substrlen == 2) {
				...
				if (s[i] != s[j] and dp[i + 1][j - 1] == 0 and dp[i + 2][j] == 0 and dp[i][j - 2] == 0) {
					dp[i][j] = 1;
				}
				else if (s[i] == s[j] and dp[i + 1][j - 1] == 0) {
					dp[i][j] = 0;
				}
				else if (dp[i + 2][j] == 1 or dp[i][j - 2] == 1)
				{
					dp[i][j] = 1;
				}
				else {
...
// outvv(dp, n);
	...
	out((dp[0][n - 1] == 1 ? "Alice" : "Draw"));	//Again confirm Bob never wins???
}
