...
void solve()
{
	...
	vector<int64> a(n), pmax(n), pqmax(n);
	inpv(a, n);


	int64 curr_pmax = LLONG_MIN, curr_pqmax = LLONG_MIN, curr_smax = LLONG_MIN;

	lp(i, n) {
		curr_pmax = max(curr_pmax, a[i] * p);
		pmax[i] = curr_pmax;

	}

	lp(i, n) {
		curr_pqmax = max(curr_pqmax, a[i] * q + pmax[i]);
		pqmax[i] = curr_pqmax;

	}

	lp(i, n) {
		curr_smax = max(curr_smax, a[i] * r + pqmax[i]);
	}
