...
void slv(dbl c, dbl m, dbl p, dbl ans1, dbl lev) {

	dbg4(c, m, p, v);

	//draw p
	ans += (dbl)ans1 * (dbl)p * (dbl)lev;
	dbg3(ans1 * p, lev, ans);
	...
	if (c == 0) {
		//draw m
		if (m > v) {
			slv(c, m - v, p + v, ans1 * m, lev + 1);
		}
		else {
			slv(c, 0, p + m, ans1 * m, lev + 1);
		}
		...
		//draw c
		if (c > v) {
			slv(c - v, m, p + v, ans1 * c, lev + 1);
		}
		else {
			slv(0, m, p + c, ans1 * c, lev + 1);
		}
	}
	else {
		//draw c
		if ((c - v) > eps) {
			slv(c - v, m + v / 2, p + v / 2, ans1 * c, lev + 1);
		}
		else {
			slv(0, m + c / 2, p + c / 2, ans1 * c, lev + 1);
		}
		//draw m
		if ((m - v) > eps) {
			slv(c + v / 2, m - v, p + v / 2, ans1 * m, lev + 1);
		}
		else {
			slv(c + m / 2, 0, p + m / 2, ans1 * m, lev + 1);
		}
	}
...
void solve()
{
	...
	ans = (double)0.0;
	slv(c, m, p, (double)1.0, (double)1.0);
	outf(ans, 12);
	return;
}
