...
void solve() {
  ...
  // Case 1: Go for second action, then first-second-first-second...
  // Case 2: Go for first action until best possible score is found, then
  // second-first-second-first...
  int maxScore = 0;
  int iter = 0;
  while (iter < 2 * n + 1 and iter < d) {
    int runningScore = 0;
    lp(i, n) { runningScore += a[i] == (i + 1); }

    dbg3(runningScore, iter, d);

    runningScore += (d - iter - 1) / 2;
