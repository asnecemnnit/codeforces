...
#define price second
	...
	vector<pair<uint64, uint64>> fence(n);
	vector<vector<uint64>> dp(n, vector<uint64>(3, LLONG_MAX));

	lp(i, n) {
		inpp(fence[i].length, fence[i].price);
	}

	dp[0][0] = 0 * fence[0].price;
	dp[0][1] = 1 * fence[0].price;
	dp[0][2] = 2 * fence[0].price;

	/*	O(n * 3 * 3) */
	lpj(i, 1, n) {
		lp(j, 3) {
			lp(k, 3) {
				if (fence[i].length + j != fence[i - 1].length + k) {

					dp[i][j] = min(dp[i][j], j * fence[i].price + dp[i - 1][k]);
