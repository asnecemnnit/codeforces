...
void solve() {
  ...
  lp(i, n) {
    // If atmost one index is remaining in current set
    if (cur.size() <= 1) {
      cout << (0) << " ";
      continue;
    }
    set<int> cur_neig, dead;

    // Iterate through all current indices (neighbours of indices which died in
    // last round)
    for (auto it = cur.begin(); it != cur.end(); ++it) {
      // Index (iterator) of current index in rem (remaining) set
      auto idx = rem.find(*it);
      // Skip if current index is already dead (not present in remaining set)
      if (idx == rem.end())
        continue;

      set<int>::iterator aft, bef;

      // Index (iterator) of alive neighbour after current index in remaining
      // set
      bef = prev(idx);
      // Index (iterator) of alive neighbour before current index in remaining
      // set
      aft = next(idx);

      // If sum of attack of neighbours before and after current index in the
      // remaining set is greater than defence of current index:
      // 1. Insert current index to dead set
      // 2. Insert index of alive neighbour before current index, and index of
      // alive neighbour after current index to cur_neig set
      if (a[*aft] + a[*bef] > d[*idx]) {
        dead.insert(*idx);
        cur_neig.insert(*bef);
        cur_neig.insert(*aft);
      }
    ...
    // Size of dead set (current indices which died in this round)
    cout << dead.size() << " ";

    // Remove all current indices which died in this round from rem (remaining)
    // set
    for (auto d : dead) {
      rem.erase(d);
    }

    // Assign cur_neig (neighbours of current indices which died in this round)
    // to current set
    cur = cur_neig;
  }
