...
int n, m, k;
const int KMAX = 10, NMAX = 500;
int dp[NMAX][NMAX][KMAX + 1];
	...
	lpj(l, 1, k + 1) {
		lp(i, n) {
			lp(j, m) {
				int mn = INT_MAX;
				if (i - 1 >= 0) {
					mn = min(mn, dp[i - 1][j][l - 1] + 2 * hor[i - 1][j]);
				}
				if (i + 1 < n) {
					mn = min(mn, dp[i + 1][j][l - 1] + 2 * hor[i][j]);
				}
				if (j - 1 >= 0) {
					mn = min(mn, dp[i][j - 1][l - 1] + 2 * ver[i][j - 1]);
				}
				if (j + 1 < m) {
					mn = min(mn, dp[i][j + 1][l - 1] + 2 * ver[i][j]);
				}
