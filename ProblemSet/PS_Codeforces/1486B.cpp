...
vector<int> findMedians(vector<int> nums) {

  // Calculate the medians
  int size = nums.size();
  vector<int> result;
  if (size % 2 == 0) {
    // If the size is even, return both middle elements
    result.pbk(nums[size / 2 - 1]);
    result.pbk(nums[size / 2]);
  } else {
    // If the size is odd, return the single middle element
    result.pbk(nums[size / 2]);
  }
...
void solve() {
  ...
  vector<int> xmedians = findMedians(x);
  vector<int> ymedians = findMedians(y);

  int64 xans = 0, yans = 0;

  if (xmedians.size() == 2) {
    xans += xmedians[1] - xmedians[0] + 1;
  } else {
    xans++;
  }

  if (ymedians.size() == 2) {
    yans += ymedians[1] - ymedians[0] + 1;
  } else {
    yans++;
  }

  // for (int xmedian : xmedians) {
  //   cout << " " << xmedian;
  // }
  // cout << nl;
  // for (int ymedian : ymedians) {
  //   cout << " " << ymedian;
  // }
  // cout << nl;
  out(xans * yans);
