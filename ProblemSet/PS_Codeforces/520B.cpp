...
void solve()
{
	...
	vector<int> visited(2 * NMAX + 1, 0);
	queue < pair < int, int>> q;
	q.push(mpr(n, 0));

	while (q.size() > 0) {
		int num = q.front().first, steps = q.front().second;
		q.pop();
		// outt(num, steps);
		...
		if (num > 0 and !visited[num - 1]) {
			visited[num - 1] = 1;
			q.push(mpr(num - 1, steps + 1));
		}

		if (num <= NMAX / 2 and !visited[2 * num]) {
			visited[2 * num] = 1;
			q.push(mpr(2 * num, steps + 1));
		}
