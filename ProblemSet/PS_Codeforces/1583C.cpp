...
void solve()
{
	...
	// Add columns (indices) which have atleast one cell whose left and top neighbors are filled/'X', in set s.
	// If this column (index) lie in query range (exclusive on left end), subgrid is indeterminable
	//
	//   1 2 3 4 5
	// 1 - - - - -
	// 2 - - X - -
	...
	// 5 - - - - -
	//
	// In above example, elements marked as 0 have left and top neighbors as filled/'X', hence 2nd and 3rd columns
	// (containinng these elements) must be added to the set. Presence of these columns in any range will make
	// subgrid indeterminable
	for (int j = 1; j < m; j++) {
		for (int i = 1; i < n; i++) {
	...
	lp(i, q) {
		...
		// Single columns are always determinable
		if (x1 == x2) {
			outy();
			continue;
		}

		// Check if no element/column (index) in set s lies in the range (x1, x2]
		if (s.upper_bound(x1) != s.upper_bound(x2)) {
			// lies in range which means indeterminable
			outn();
		}
		else {
			// does not lie in range which means determinable
			outy();
		}
