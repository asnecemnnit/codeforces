...
void solve() {
  ...
  if ((xb - xa) % 2 == 0) {
    // Either Bob wins on his turn or draw
    int amoves = (xb - xa) / 2, bmoves = (xb - xa) / 2;

    int aymin = max(1, ya - amoves), aymax = min(w, ya + amoves);
    int bymin = max(1, yb - bmoves), bymax = min(w, yb + bmoves);
    ...
    // Either Alice wins on his turn or draw
    int amoves = (xb - xa) / 2 + 1, bmoves = (xb - xa) / 2;
