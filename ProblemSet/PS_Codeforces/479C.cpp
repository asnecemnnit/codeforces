...
{
	inp(n);
	vector<pair<uint32, uint32>> ab(n);
	lp(i, n) {
	...
	int last_day = min(ab[0].first, ab[0].second);
	lpj(i, 1, n) {
		// cout << ab[i].first << " " << ab[i].second << nl;
		if (min(ab[i].first, ab[i].second) >= last_day) {
			last_day = min(ab[i].first, ab[i].second);
		}
		else {
			last_day = max(ab[i].first, ab[i].second);
		}
	}
