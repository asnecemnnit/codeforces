...
void solve()
{
	...
	uint64 xorTillNow = a[0], lastElem = a[0];
	bool allElemsAreSame = true;

	lpj(i, 1, n) {
		xorTillNow = xorTillNow ^ a[i];
		allElemsAreSame = allElemsAreSame and (a[i] == lastElem);
		lastElem = a[i];
	}

	// out(xorTillNow);
	if (allElemsAreSame or xorTillNow == 0) {
		outy();
	}
	else {
		outn();
