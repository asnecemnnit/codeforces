...
void dfs(int u, int v) {
	visited[u] = true;
	vector<int> u_adj = adj[u].second;
	int left = adj[u].first.first, right = adj[u].first.second;
	lp(i, u_adj.size()) {
		int next = u_adj[i];

		if (next != v and !visited[next]) {
			dfs(next, u);
			int child_left = adj[next].first.first, child_right = adj[next].first.second;

			dp[u][0] += max(dp[next][0] + abs(left - child_left), dp[next][1] + abs(left - child_right));
			dp[u][1] += max(dp[next][0] + abs(right - child_left), dp[next][1] + abs(right - child_right));
			dbg2(u, next);
			dbg4(dp[u][0], dp[u][1], dp[next][0], dp[next][1])
