...
const int NMAX = 51;
int n, m;
int visited1[NMAX][NMAX], visited2[NMAX][NMAX];
const int nb[4][2] = {{0, -1}, {0, 1}, {-1, 0}, {1, 0}};
int dp1[NMAX][NMAX], dp2[NMAX][NMAX];

/************************** User-Defined Functions ***************************/
bool BFS1(vector<vector<char>> &b, int n, int m) {
  lp(i, n) {
    lp(j, m) {
      if (b[i][j] != 'B')
        continue;
      ...
      while (q.size() > 0) {
        ...
        lp(k, 4) {
          int nbx = x + nb[k][0], nby = y + nb[k][1];

          if (nbx == -1 or nby == -1 or nbx == n or nby == m) {
            continue;
          }
          ...
          if (visited1[nbx][nby] == -1 and (b[nbx][nby] == 'B')) {
            visited1[nbx][nby] = 1;
            q.push(make_pair(nbx, nby));
          }
        }
      }
    }
...
bool BFS2(vector<vector<char>> b, int n, int m) {
  lp(i, n) {
    lp(j, m) {
      if (b[i][j] != 'G')
        continue;
      ...
      while (q.size() > 0) {
        ...
        if (x == n - 1 and y == m - 1 and (b[x][y] == 'G' or b[x][y] == '.')) {
        ...
        lp(k, 4) {
          ...
          if (b[nbx][nby] == '#' or b[nbx][nby] == 'B') {
            continue;
          }

          if (visited2[nbx][nby] == -1 and
              (b[nbx][nby] == 'G' or b[nbx][nby] == '.')) {
            visited2[nbx][nby] = 1;
            q.push(make_pair(nbx, nby));
          }
        }
...
void solve() {
  ...
  bool gpresent = false, bpresent = false;
  ...
  // Process all Bs, modify neighbouring empty cells (.) with walls (#)
  if (!BFS1(a, n, m)) {
    outn();
    return;
  }

  // Process all Gs, modify neighbouring empty cells (.) with Gs
  BFS2(a, n, m) == true ? outy() : outn();
