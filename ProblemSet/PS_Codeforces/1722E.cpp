...
const int NMAX = 1001;
uint64 dp[NMAX][NMAX]; // // dp[i][j] = count of entries with h=i and w=j
uint64 dpa[NMAX][NMAX]; // dpa[i][j] = sum of areas of rectangles with h<i and w<j
int n, q;
...
void solve()
{
	...
	lpj(i, 1, 1001) {
		lpj(j, 1, 1001) {
			dpa[i][j] = dp[i][j] * i * j + dpa[i - 1][j] + dpa[i][j - 1] - dpa[i - 1][j - 1];
	...
	lp(i, q) {
		...
		uint64 ans = dpa[hb - 1][wb - 1] - dpa[hs][wb - 1] - dpa[hb - 1][ws] + dpa[hs][ws];
		out(ans);
	}
