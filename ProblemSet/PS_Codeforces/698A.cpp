...
int n, m;
int dp[NMAX][3];		/*
							dp[i][0]	->	minimum rest days till ith day (being a rest day)
							dp[i][1]	->	minimum rest days till ith day (being a contest day)
							dp[i][2]	->	minimum rest days till ith day (being a gym/sports day)
						*/
...
void solve()
{
	...
	dp[0][2] = 0;
	lpj(i, 1, n + 1) {
		/*	dp[i][j] -> INT_MAX means it is not possible for ith day to be a 'j' type day */
		dp[i][0] = INT_MAX;
		dp[i][1] = INT_MAX;
		dp[i][2] = INT_MAX;
	}
	...
	lpj(i, 1, n + 1) {
		int min_rest_prev = min({dp[i - 1][0], dp[i - 1][1], dp[i - 1][2]});

		if (a[i - 1] == 3) {
			// dp[i][0] = min(dp[i - 1][1], dp[i - 1][2]) + 1;
			dp[i][0] = min_rest_prev + 1;
		...
		dbg4(a[i - 1], dp[i][0], dp[i][1], dp[i][2]);
	}
