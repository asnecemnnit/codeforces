...
void solve()
{
	...
	int nbits = n == 0 ? 0 : floor(log2(n) + 1), mbits = m == 0 ? 0 : floor(log2(m) + 1);
	...
	if (m == ((uint64)((uint64)1 << (uint64)mbits) - 1)) {
		out(((uint64)((uint64)1 << (uint64)mbits) - 1));
	}
	else
		out((uint64)((uint64)1 << (uint64)(mbits - 1)) - 1);
