...
void solve()
{
    ...
    REP(i, n){
        if(s[i] == 'h')
            st[s[i]]++;
        else if(s[i] == 'e' and st.find('h') != st.end())
            st[s[i]]++;
        else if(s[i] == 'l' and st.find('h') != st.end() and st.find('e') != st.end())
            st[s[i]]++;
        else if(s[i] == 'o' and st.find('h') != st.end() and st.find('e') != st.end() and st['l'] >= 2)
            st[s[i]]++;
    }

    st.size() == 4 ? OUT("YES") : OUT("NO");
    return;
