...
void solve()
{
	...
	if (n % 2 != 0) {
		...
		uint64 anse = 0, anso = 0;
		vi64 se, so;
		// Every even indexed building
		...
		ans = min(anse, anso);
		// Case where there is an extra building in between
		// which is not a peak(mix of even and
		// odd indexed buildings)

		vi64 rse;
		uint64 ranse = 0;

		// mixed of even index followed by reversed even index could result in highest peaks
		...
		// mixed of odd index followed by reversed odd index would result in 1 less highest peak
		...
		for (int i = 0; i < pSize - 1; i++) {
			ans = min(ans, se[i] + rse[pSize - i - 2]);
		}
