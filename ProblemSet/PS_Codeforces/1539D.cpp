...
#define disc_num first
#define req_num second

	vector<pair<uint64, uint64>> ab(n);

	lp(i, n) {
		inpp(ab[i].req_num, ab[i].disc_num);
	}
	...
	while (l <= r) {
		if (tot >= ab[l].disc_num) {
			ans += ab[l].req_num;
			tot += ab[l].req_num;
			l++;
		}
		else {
			uint64 req = ab[l].disc_num - tot;
			if (req < ab[r].req_num) {
				ans += 2 * req;
				tot += req;
				ab[r].req_num -= req;
