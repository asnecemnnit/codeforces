...
void solve()
{
	...
	sort(all(rem));
	// outv(rem, n);
	auto iter = lower_bound(rem.begin(), rem.end(), 0);
	// int64 non_neg = int64(iter - rem.begin());
	// out(non_neg);

	int inflex = int(iter - rem.begin());
	// out(inflex);
	int start = inflex - 1, end = inflex;
	int count0 = 0;
	int64 bal = 0;
	...
	while (start >= 0 and end < n) {
		...
		else if (bal + rem[start] + rem[end] < 0) {
