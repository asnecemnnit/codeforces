...
void solve(int tc)
{
	...
	char start = s[0], end = s[slen - 1];
	if (start == end and s.length() == 2) {
		seq.pbk(1);
		seq.pbk(2);
		outt(0, 2);
		outv(seq, seq.size());
	...
	if (start == end) {
		...
		if (sid[0].first == start) {
			lp(i, sid.size()) {
				if (sid[i].first != start) {
					break;
				}
				else {
					seq.pbk(sid[i].second);
				}
			}
		}
		else {
		...
		lp(i, sid.size()) {
			...
			else if (sid[i].first < start)
				continue;
			else {
				seq.pbk(sid[i].second);
				mincost += abs(sid[i].first - curr);
				curr = sid[i].first ;
			}
		}
	}
		...
		rlp(i, sid.size() - 1) {
			...
			else if (sid[i].first < end)
				break;
			else {
				seq.pbk(sid[i].second);
				mincost += abs(curr - sid[i].first);
				curr = sid[i].first ;
			}
		}
	}
	...
	mincost += abs(end - curr);


	outt(mincost, seq.size());
	outv(seq, seq.size());
