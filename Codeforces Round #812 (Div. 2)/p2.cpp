...
bool arraySortedAscending(vector<int> arr, int n)
{
...
bool arraySortedDescending(vector<int> arr, int n)
{
	// Array has one or no element
	if (n == 0 || n == 1)
		return true;
...
void solve()
{
	...
	if (arraySortedAscending(a, a .size()) or (arraySortedDescending(a, a.size()))) {
		outy();

	}
	else if (isMountainArray(a)) {
		outy();
	}
	else {
		outn();
