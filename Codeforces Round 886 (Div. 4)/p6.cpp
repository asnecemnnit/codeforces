...
void solve() {
  ...
  // Update star values using frequency information
  for (int i = 1; i <= N; i++) {
    for (int j = i; j <= N; j += i) {
      starValue[j] += frequency[i];
    }
  }
  ...
  // Find and print the  secondmaximum star value
  int maxStarValue = *max_element(starValue.begin(), starValue.end());
