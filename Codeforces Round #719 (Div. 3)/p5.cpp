...
void solve()
{
    ...
    if(sz % 2 == 0)
    {
        int md1 = sz / 2 - 1, md2 = sz / 2;


        uint64 ans1 = 0;

        int left = idx[md1] - 1, right = idx[md1] + 1;

        for(int i = md1 - 1; i >= 0; i--)
        {
            ans1 += (uint64)(left - idx[i]);
            left--;
        }

        for(int i = md1 + 1; i < sz; i++)
        {
            ans1 += (uint64)(idx[i] - right);
            right++;
        }
        ...
        left = idx[md2] - 1, right = idx[md2] + 1;

        for(int i = md2 - 1; i >= 0; i--)
        {
            ans2 += (uint64)(left - idx[i]);
            left--;
        }

        for(int i = md2 + 1; i < sz; i++)
        {
            ans2 += (uint64)(idx[i] - right);
            right++;
        }
        ...
        int left = idx[md] - 1, right = idx[md] + 1;

        for(int i = md - 1; i >= 0; i--)
        {
            ans += (uint64)(left - idx[i]);
            left--;
        }

        for(int i = md + 1; i < sz; i++)
        {
            ans += (uint64)(idx[i] - right);
            right++;
        }
