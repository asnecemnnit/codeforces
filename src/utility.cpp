...
/* Minimum Adjacent Swaps needed to get another target permutation string s2 of
 * string s1 */
...
/* Returns SCS (shortest common supersequqnce, not just the length) for str1 and
 * str2 */
...
void shortest_path_unweighted() {
  ...
  REP(i, pred_path.size()) { cout << pred_path[i] << " "; }
  cout << "\n";
  return;
}
...
/*	Simple & efficient Dijkstra	O(n+mlogm) because
        ...
        the priority queue
        - Greedy Approach.
        - Faster in practice on sparse graphs (where the number of edges is much
...
/*	Simple Bellman Ford	O(nm)
        ...
        - Not suitable for graphs with negative weight cycles (as it may not
...
vector<int> dist[N + 1]; // dist[] contains minimum distance from 'x' to all
                         // nodes in the graph
 ...
 */
void addEdge(int u, int v, bool isWeighted, int wt) {
  if (isWeighted) {
...
void createGraph(int n, bool isWeighted) {
  ...
  for (int i = 0; i < v.size(); i++) {
    addEdge(v[i].second.first, v[i].second.second, isWeighted, v[i].first);
  }
...
void distinctElementsInCurrentRange() {
  ...
  for (int i = 0; i < m; i++) {
    ...
    cout << "Number of distinct elements in [" << L << ", " << R << "] is "
...
int query(int node, int start, int end, int l, int r, string op) {
  ...
  int mid = (start + end) / 2;
  return outOp(op, query(2 * node, start, mid, l, min(r, mid), op),
               query(2 * node + 1, mid + 1, end, max(l, mid + 1), r, op));
}
...
void updateRangeLazy(int node, int start, int end, int l, int r, int val) {
  ...
  updateRangeLazy(node << 1 | 1, mid + 1, end, l, r,
                  val);                               // Updating right child
...
void assignRangeLazy(int node, int start, int end, int l, int r, int val) {
  ...
  assignRangeLazy(node << 1 | 1, mid + 1, end, l, r,
                  val);                               // Updating right child
  tree[node] = tree[node << 1] + tree[node << 1 | 1]; // Updating root
}
...
uint64 queryRangeLazy(int node, int start, int end, int l, int r) {
  ...
  return queryRangeLazy(node << 1, start, mid, l, r) +
         queryRangeLazy(node << 1 | 1, mid + 1, end, l, r);
}
...
public:
  ...
  int countSets() { return sets.size(); }
