...
bool bfs(int n, int m, int k)
{
	vector<vector<bool>> visited(n, vector<bool>(m, false));
	queue<pair<long, pair<int, int>>> q;
	...
	while(!q.empty())
	{
		...
		long cost = q.front().first;
		int x = xy.first;
		int y = xy.second;
		q.pop();

		if(x == n-1 and y == m-1 and cost == k)
			return true;
		...
		if(x < n - 1 and !visited[x + 1][y] and (cost + y + 1 <= k))
		{
			visited[x+1][y] = true;
			q.push(make_pair(cost + y + 1, make_pair(x+1, y)));
		}

		if(y < m - 1 and !visited[x][y+1] and (cost + x + 1 <= k))
		{

			visited[x][y+1] = true;
			q.push(make_pair(cost + x + 1, make_pair(x, y+1)));
		}
