...
uint64 sumOfProd(vector<uint64> a, vector<uint64> b, int n)
{
	uint64 sum = 0;
...
vector<uint64> reverse_new(vector<uint64> a, int n)
{
	for(int i=0; i<n/2; i++)
	{
		uint64 temp = a[i];
...
void solve()
{
	...
	vector<uint64> pre_sp(n);
	pre_sp[0] = a[0] * b[0];
	for(int i=1; i<n; i++)
	{
		pre_sp[i] += pre_sp[i-1] + a[i] * b[i];
	}

	uint64 max_sp = pre_sp[n-1];
	...
	for(int i=1; i<n; i++)
	{
		...
		{
			int j = i-1;
			while(i<n and a[i] >= a[i-1] and b[i] <= b[i-1])
			{
				i++;
			}
			i--;
		    ...
		    auto first = b.begin() + j;
		    auto last = b.begin() + i + 1;

		    // Copy the element
		    vector<uint64> bslice(first, last);

			first = a.begin() + j;
		    last = a.begin() + i + 1;
			vector<uint64> aslice(first, last);
			vector<uint64> aslice_rev = reverse_new(aslice, i-j+1);


			max_sp = max(max_sp, j == 0 ? 0 : pre_sp[j-1] +
				 sumOfProd(aslice_rev,
				 bslice, i-j+1) + pre_sp[n-1] - pre_sp[i]);
			OUT(max_sp);
