...
// 		lis[i] = 1;
// 		for (int j = 0; j < i; j++ ) {
// 			if (((nums[i] > dp[j] and abs(nums[i] - dp[j]) >= nums[i])) and lis[i] < lis[j] + 1) {
// 				// dp[i] = nums[i];
// 				lis[i] = lis[j] + 1;
// 			}
// 			else if (((nums[i] < dp[j] and abs(nums[i] - dp[j]) >= dp[j])) and lis[i] < lis[j] + 1) {
// 				dp[i] = dp[j];
// 				lis[i] = lis[j] + 1;
// 			}
// 		}
...
void solve()
{
	...
	if (0 >= *max_element(all(a))) {
		OUT(n);
		return;
	}

	if (0 < *min_element(all(a))) {
		OUT(1);
		return;
	}

	if (*max_element(all(a)) == 0 and * min_element(all(a)) == 0) {
		OUT(n);
		return;
	}
