...
void solve()
{
	...
	REP(i, n) {
		...
		if (!st.empty())
		{
			...
			if (tp.first != a[i]) {
				if (a[i] == 1)
					mp1[i] = abs(tp.second - i);
				else
					mp1[tp.second] = abs(tp.second - i);
	...
	for (int i = n - 1; i >= 0; i--) {
		if (!st1.empty())
		{
			...
			if (tp.first != a[i]) {
				// dbg2(abs(tp.second - i), fwd[n1 - 1 - start]);
				if (a[i] == 1) {
					int mn = min(mp1[i], abs(tp.second - i));
					if (i + mn == 0) {
						if (mp2[i + mn] > 0) {
							ans += (uint64)(mp1[i] + abs(tp.second - i) - mn);
							mp2[i + (mp1[i] + abs(tp.second - i) - mn)]++;
						}
						else {
							ans += (uint64)mn;
							mp2[i + mn]++;
					...
					int mn = min(mp1[tp.second], abs(tp.second - i));
					if (i + mn == 0) {
						if (mp2[i + mn] > 0) {
							ans += (uint64)(mp1[tp.second] + abs(tp.second - i) - mn);
							mp2[i + (mp1[tp.second] + abs(tp.second - i) - mn)]++;
						}
						else {
							ans += (uint64)mn;
							mp2[i + mn]++;
