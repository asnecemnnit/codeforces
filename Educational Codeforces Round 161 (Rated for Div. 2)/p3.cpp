...
void solve() {
  ...
  for (int i = 1; i < n; i++) {
    if (i == 1) {
      ...
      dist[i] = dist[i - 1] + ((abs(a[i - 1] - a[i - 2]) > abs(a[i] - a[i - 1]))
                                   ? 1
                                   : abs(a[i] - a[i - 1]));
    }
  }
  ...
  for (int i = n - 2; i >= 0; i--) {
    if (i == n - 2) {
      ...
      rdist[i] =
          rdist[i + 1] + ((abs(a[i + 1] - a[i + 2]) > abs(a[i] - a[i + 1]))
                              ? 1
                              : abs(a[i] - a[i + 1]));
    }
  }
  ...
  while (m--) {
    int x, y;
    inpp(x, y);
    out((x < y ? dist[y - 1] - dist[x - 1] : rdist[y - 1] - rdist[x - 1]));
  }
  return;
}
